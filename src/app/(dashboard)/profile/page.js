...
export default function ProfilePage() {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    setPasswordError("");
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/auth/student/me/password`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/auth/student/me/upload-image`, {
        method: "POST",
        credentials: "include",
        body: formData,
      });
