...
export default function CourseTopicsPage({ params }) {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/student/me/course/${code}`, {
          credentials: "omit"
        });
        if (res.ok) {
          ...
          const res2 = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/student/me/course/${code}`, {
            credentials: "include"
          });
          if (res2.ok) {
  ...
  const handleToggleTopic = async (topicId) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/student/me/topic/${topicId}/toggle`, {
        method: "POST",
        credentials: "include",
      });
      if (res.ok) {
